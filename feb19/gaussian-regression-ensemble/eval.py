...
networks = []
for i in range(M):
    network = ToyNet("eval_gaussian-regression-ensemble_1", project_dir="/root/blog/feb19/gaussian-regression-ensemble").cuda()
    network.load_state_dict(torch.load("/root/blog/feb19/gaussian-regression-ensemble/training_logs/model_gaussian-regression-ensemble_1_%d/checkpoints/model_gaussian-regression-ensemble_1_epoch_75.pth" % i))
